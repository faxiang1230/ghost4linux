...
//new var für g4l
static float actsum = 0;
static unsigned long long sfsize=0;
static unsigned long long bfsize=0;
static float perc=0;
...
static void usage(char *argv0)
{
  ...
  "  -p, --percent <kbyte>\t\tfor percentage (g4l) enter data to read in kByte\n"
...
// we want actual sum here, so:
	actsum+=(((float)cntout-(float)statout)/1024.0);
  perc=(actsum/(float)bfsize)*100.0;
// time_offset is used to handle problem if the time goes past midnight
if (tv->tv_sec < beginning.tv_sec)
		time_offset = 3600L * 24;
else
		time_offset = 0;
mbrate = (actsum/1024)/(long)(time_offset + tv->tv_sec - beginning.tv_sec);
fprintf(stderr, "%.2f%% \t%9.2fMB of %.2fMB \ttime: %u:%02u:%02u  %.2fMB/sec\n", 
		perc, actsum/1024, (float)bfsize/1024,
    (time_offset + tv->tv_sec - beginning.tv_sec) / 3600,
    (time_offset + tv->tv_sec - beginning.tv_sec) / 60 % 60,
    (time_offset + tv->tv_sec - beginning.tv_sec) % 60, mbrate);
	statin = cntin;
	statout = cntout;
}
...
static void readopts(int argc, char *argv[])
{
  ...
  while (a > 0) {
    ...
    switch (c) {
      ...
      case 'p':
        bfsize = optamount(dataunits, datamultipliers, v[0], argv[0]);
        v += 1;
        a -= 1;
        break;
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    if ((nfds > 0) && (pfd[1].revents)) {
      ...
      if (n == 0) {
// Added to print final statistics
				statistics(&now, cntin, cntout);
        finalstat();
        return 0;
      }
