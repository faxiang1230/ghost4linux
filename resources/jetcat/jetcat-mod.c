...
static unsigned long long actsum2 = 0;
...
static void getactsum(float bigsum)
{
  actsum+=bigsum;
...
static void getpercent(float sofar)
{
    perc=(sofar/(float)bfsize)*100.0;
}
...
//we want acutal sum here, so:
getactsum(((float)cntout-(float)statout)/1024.0);
getpercent(actsum);



if (actsum2 > 0) {
//	fprintf(stderr, "%llu%019llu\n", actsum2, actsum);
fprintf(stderr, "%.2f\n", perc);
	statin = cntin;
	statout = cntout;

  } else {
//	fprintf(stderr, "SUM: %.2fk, -p PARA: %lluk, PEC: %.2f%\n", actsum, bfsize, perc);
fprintf(stderr, "%.2f\n", perc);
	statin = cntin;
	statout = cntout;
  }
